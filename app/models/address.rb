class Address < ActiveRecord::Base
  ...
  def self.find_addresses_with_cases_within_area(ne, sw)
    ...
    box.add(p1).add(p2)
    @addresses = Address.find_by_sql("SELECT a.* FROM addresses a INNER JOIN cases c ON c.address_id = a.id WHERE ST_Within(point, ST_GeomFromText('#{box.to_geometry.as_text}')) GROUP BY a.id, a.geopin ORDER BY a.street_name ASC, a.house_num ASC;")
  end
end
